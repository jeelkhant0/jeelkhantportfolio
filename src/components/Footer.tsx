...
export default function Footer() {
  return (
    <footer className="section-shell pt-0">
      <div className="mx-auto max-w-7xl rounded-xl border border-brand/30 bg-gradient-to-r from-brand/15 via-surface to-surface p-6 shadow-soft md:p-8">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-brand">Next Step</p>
            <h2 className="mt-2 text-2xl font-semibold text-text">{siteConfig.footerCta.title}</h2>
            <p className="mt-2 text-sm text-muted">{siteConfig.footerCta.description}</p>
          </div>
          <Link
            href={siteConfig.footerCta.href}
            className="rounded-xl bg-brand px-5 py-3 text-sm font-semibold text-white transition duration-300 hover:-translate-y-0.5 hover:bg-brandStrong"
          >
      ...
      <div className="mx-auto mt-8 flex max-w-7xl flex-col items-center justify-between gap-4 border-t border-border pt-6 text-sm text-muted md:flex-row">
        ...
        <div className="flex items-center gap-5">
          {siteConfig.socialLinks.map((item) => (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="link-sweep inline-flex items-center gap-2 transition hover:text-text"
            >
